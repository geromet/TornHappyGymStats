...
security() {
  bash "$CLASSIFIER" --security-boundary --files "$@"
}
...
assert_eq "mixed Razor + migration + remote returns the union" \
  $'T2\nT3\nT4' \
  "$(classify src/HappyGymStats.Blazor/Pages/War.razor src/HappyGymStats.Data/Migrations/20260904_Test.cs scripts/remote-exec.sh)"

assert_eq "one path may require more than one stronger tier" \
  $'T2\nT4' \
  "$(classify src/HappyGymStats.Data/Views/RelationalStatus.razor)"

assert_eq "ordinary source leaves security boundary unchanged" \
  'security_boundary=unchanged' \
  "$(security src/HappyGymStats.Core/Models/Foo.cs)"
assert_eq "import ownership marks security boundary changed" \
  'security_boundary=changed' \
  "$(security src/HappyGymStats.Api/Controllers/ImportController.cs)"
assert_eq "nginx/infra marks security boundary changed" \
  'security_boundary=changed' \
  "$(security infra/nginx-torn.conf)"

assert_eq "machine-readable JSON includes both policy dimensions" \
  '{"tiers":["T2","T4"],"security_boundary":"unchanged"}' \
  "$(bash "$CLASSIFIER" --format json --files src/HappyGymStats.Blazor/Pages/War.razor src/HappyGymStats.Data/Queries/PostgresQuery.cs)"

# Exceptions/rules without a checked-in reason must be rejected.
tmp_rules="$(mktemp)"
...
printf 'pattern\ttier\tsecurity\treason\n*.razor\tT2\tmaybe\ttest\n' > "$tmp_rules"
if EVIDENCE_RULES_FILE="$tmp_rules" bash "$CLASSIFIER" --files Foo.razor >/tmp/hgs-evidence-out.$$ 2>/tmp/hgs-evidence-err.$$; then
  echo "FAIL: invalid security flag was accepted" >&2
  ((failures += 1))
elif grep -q 'invalid security flag' /tmp/hgs-evidence-err.$$; then
  echo "PASS: invalid security flag is rejected"
else
  echo "FAIL: invalid security flag failed for the wrong reason" >&2
  cat /tmp/hgs-evidence-err.$$ >&2
  ((failures += 1))
fi
