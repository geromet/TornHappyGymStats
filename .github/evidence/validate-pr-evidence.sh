...
while (($#)); do
  case "$1" in
    --body-file)
      (($# >= 2)) || { echo "ERROR: --body-file requires a path" >&2; exit 2; }
      body_file="$2"
      shift 2
      ;;
    --base)
...
block="$(awk '
  ...
  inside { print }
  END {
    if (found != 1 || inside || duplicate) exit 42
  }
' "${body_file}")" || {
...
field() {
  ...
  [[ "${count}" == "1" ]] || {
    echo "ERROR: evidence field '${key}' must appear exactly once" >&2
    return 1
  }
  value="$(printf '%s\n' "${block}" | sed -nE "s/^${key}:[[:space:]]*//p")"
  [[ -n "${value}" ]] || {
    echo "ERROR: evidence field '${key}' may not be empty" >&2
    return 1
  }
  printf '%s' "${value}"
}

for required_field in task lease required observed unverified regression tier2 tier3 tier4; do
...
normalize_tiers() {
  ...
  for token in "${tokens[@]}"; do
    [[ "${token}" =~ ^T[1-4]$ ]] || {
      echo "ERROR: invalid evidence tier '${token}' in '${raw}'" >&2
      return 1
    }
    seen["${token}"]=1
  done
...
if [[ "${computed_csv}" != "none" ]]; then
  ...
  for tier in "${required_tiers[@]}"; do
    if contains_tier "${observed}" "${tier}"; then
      if contains_tier "${unverified}" "${tier}"; then
        echo "ERROR: ${tier} cannot be both observed and unverified" >&2
        exit 1
      fi
      continue
    fi
...
if [[ "${unverified}" != "none" ]]; then
  IFS=',' read -r -a unverified_tiers <<< "${unverified}"
  for tier in "${unverified_tiers[@]}"; do
    if ! contains_tier "${computed_csv}" "${tier}"; then
      echo "ERROR: ${tier} is marked unverified but is not required by this diff" >&2
      exit 1
    fi
  done
fi

# Stronger evidence needs a concrete pointer or an explicit pending/unverified
# explanation. `n/a` is only valid when that tier is not required.
check_detail() {
  local tier="$1" detail="$2"
  if contains_tier "${computed_csv}" "${tier}"; then
    [[ "${detail}" != "n/a" ]] || {
      echo "ERROR: ${tier} is required; its detail field may not be n/a" >&2
      return 1
    }
  fi
}
...
printf 'PR EVIDENCE PASS — task=%s lease=%s required=%s observed=%s unverified=%s\n' \
  "${task}" "${lease}" "${computed_csv}" "${observed}" "${unverified}"
