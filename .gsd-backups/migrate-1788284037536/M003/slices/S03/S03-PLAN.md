...
- [x] **T01: Defined a least-privilege AdminPanel sudoers boundary with explicit steady-state vs bootstrap-only commands and documented it in deployment guidance.** `est:1h`
...
- [x] **T02: Added an idempotent AdminPanel server setup script that stages and installs the systemd unit, reloads/enable-starts the service, and verifies /admin/health with named fail-fast phases.** `est:2h`
...
- [x] **T03: Added fail-fast sudoers staging, visudo validation, and 0440 install phases to AdminPanel setup before service activation.** `est:1.5h`
...
- [x] **T04: Added explicit AdminPanel service/loopback health classification to setup plus an S03 local verifier and deployment guidance for confirmation before remote mutation.** `est:1h`
...
- [ ] **T05: Harden AdminPanel setup privilege boundary and diagnostics** `est:1.5h`
  ...
  - Files: `infra/sudoers-happygymstats`, `scripts/setup-adminpanel-server.sh`, `scripts/verify/s03-adminpanel-setup.sh`, `docs/DEPLOYMENT.md`
