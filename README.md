...
- `GET /api/v1/torn/gym-trains?limit=5`
- `GET /api/v1/torn/happy-events?limit=5`
- `POST /api/v1/torn/import-jobs`
- `GET /api/v1/torn/import-jobs/latest`

The frontend calls the API through same-origin routing at `/api/*` on `torn.geromet.com`.
...
The frontend now uses same-origin `/api/*` paths in production (`https://torn.geromet.com/api/...`).
For local preview you can either:

- run nginx/proxy mapping `/api` to `http://localhost:5047`, or
- temporarily edit `web/app.js` API base for local-only testing.
...
Host deployment targets:

- Frontend: `https://torn.geromet.com`
- API: `https://torn.geromet.com/api/...` (proxied to local API service)
...
- uploads `web/` to a timestamped release directory
- flips `/var/www/torn-frontend/current`
- expects nginx to serve `torn.geromet.com` and proxy `/api/*` to `127.0.0.1:5047`
...
- `torn.geromet.com` serves frontend static assets from `/var/www/torn-frontend/current`.
- `/api/*` on `torn.geromet.com` is reverse-proxied by nginx to `http://127.0.0.1:5047`.
- API routes are namespaced under `/api/v1/torn/*`.

## Tech stack

- .NET 8, C# 12
- ASP.NET Core minimal API + EF Core + SQLite
- Static HTML/CSS/JavaScript dashboard served by nginx
