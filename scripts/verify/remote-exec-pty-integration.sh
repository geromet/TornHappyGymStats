...
reset; O="$D/old-stdin.out"; set +e; printf 'echo HISTORICAL_PAYLOAD\n' | timeout 4s ssh -tt -i "$KEY" -p "$PORT" -o StrictHostKeyChecking=yes -o "UserKnownHostsFile=$KH" "$U@127.0.0.1" 'sudo -k; sudo -v && bash -s' >"$O" 2>&1; r=$?; set -e; ((r==124)) && pass 'historical stdin collision bounded by timeout' || fail "historical stdin collision expected timeout rc=124 actual=$r"
